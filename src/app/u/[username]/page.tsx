...
import { error } from 'console';
 ...
 const initialContent = "What's your favorite movie?||Do you have any pets?||What's your dream job?";
 ...
 const {complete, completion, isLoading: isSuggestLoading} = useCompletion({
  ...
  initialCompletion: initialContent
 })
 ...
 const onSubmit = async (data: z.infer<typeof MessagesSchema>) => {
  ...
  try {
    await dbConnect()
   const response = await axios.post('/api/suggest-messages', {data, username});
   toast({
    ...
    toast({
      title: 'error',
      description: ApiError.response?.data?.message ?? "Failed to send message",
      variant: "destructive"
    })
  }finally{
    setIsLoading(false)
